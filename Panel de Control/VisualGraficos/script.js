...
function eliminarDato(elemento) {
    // Obtén el elemento padre del elemento a eliminar
    const datos = document.getElementById("datos");
    
    // Elimina el elemento
    datos.removeChild(elemento);
}

function agregarDato() {
    // Tomo la cantidad de leyendas actual
    const cantidadLeyendas = document.getElementsByClassName("dato").length + 1;
    ...
    // Creo un botón de eliminación y le asigno sus propiedades
    const botonEliminar = document.createElement("button");
    botonEliminar.textContent = "-";
    // Asigno un manejador de evento para eliminar el elemento cuando se hace clic en el botón
    botonEliminar.addEventListener("click", function() {
        eliminarDato(dato);
    });
    // Agrego el botón al div datos
    dato.appendChild(botonEliminar);

    // Agrego el div dato al contenedor de datos
    document.getElementById("datos").appendChild(dato);
}

const botonEliminar = document.getElementById('eliminarBoton');

// Aplica un estilo al botón
botonEliminar.style.backgroundColor = 'red'; // Cambia el color de fondo a rojo
botonEliminar.style.color = 'white'; // Cambia el color del texto a blanco
botonEliminar.style.padding = '10px 20px'; // Cambia el padding
